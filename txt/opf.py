"""Calibre .opf sidecar detection and <metadata> extraction for --ingest."""
...
def find_opf_sidecar(path: Path) -> Path | None:
    """The sibling <name>.opf (any case) for a <name>.epub (any case) file."""
    if not path.name.lower().endswith(_EPUB_SUFFIX):
        logger.debug("%s: not a .epub file, skipping OPF lookup", path)
        return None
    base = path.name[: -len(_EPUB_SUFFIX)]
