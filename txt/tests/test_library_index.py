    ...
    def query(self, sql, params=None):
        ...
        return [
            (
                txt_id,
                (
                    brotli.compress(json.dumps(metadata).encode())
...
def test_library_index_round_trips_docs_and_terms(engine):
    ...
    docs = [
        (
            ...
            {
                "title": "Book One",
                "creator": [{"text": "Author A"}, {"text": "Author B"}],
                "subject": "Fiction",
            },
        ),
        (
            ...
            {
                "title": "Book Two",
                "calibre:title_sort": "Book Two Sorted",
                "publisher": "Acme",
            },
        ),
        (3, None),
    ...
    rows = conn.execute(
        "SELECT txt_id, title, sort_key FROM doc ORDER BY txt_id"
    ).fetchall()
    assert rows == [
        (1, "Book One", "Book One"),
        (2, "Book Two", "Book Two Sorted"),
        (3, "", None),
    ]
    ...
    built = conn.execute(
        "SELECT built_at_version, doc_count FROM built WHERE id = 1"
    ).fetchone()
    assert built == (7, 3)
    conn.close()
...
def test_library_index_interns_shared_terms(engine):
    ...
    assert len(terms) == 1
    doc_terms = conn.execute(
        "SELECT doc_id, term_id FROM doc_term ORDER BY doc_id"
    ).fetchall()
...
def test_library_index_wrong_key_fails_to_decrypt(engine):
    builder = LibraryIndexBuilder(
        FakeBB([(1, {"title": "Book"})]), CryptoBlob(engine), secrets.token_bytes(128)
    )
    encrypted, _doc_count, _hash = builder.build(built_at_version=1)
