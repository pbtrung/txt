...
INSERT_USER_SQL = "INSERT INTO users (id, type, created_at) VALUES (?, ?, ?)"
...
class AdminInitializer:
    ...
    def _insert_user(self, ctl: LibsqlClient, uid: str) -> None:
        created_at = int(time.time() * 1000)
        self.logger.verbose(f"Inserting users row: id={uid}, type=admin, created_at={created_at}...")
        ctl.execute(INSERT_USER_SQL, [uid, "admin", created_at])
        self.logger.verbose("users row inserted.")
        self.logger.info(f"Registered admin {uid} in ctl")
