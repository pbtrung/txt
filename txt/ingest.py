...
class TxtIngester:
    ...
    def run(self) -> None:
        ...
        self.db_master_key = self._require_db_master_key(
            aa, uid, account_type, self.umk
        )
    ...
    def _ingest_all(self, aa: LibsqlClient) -> None:
        ...
        epub_paths = sorted(
            p for p in self.input_dir.glob("*.epub") if p.name not in existing
        )
        self.logger.verbose(
            f"{len(epub_paths)} file(s) to ingest, {len(existing)} already done"
        )
    ...
    def _ingest_file(self, aa: LibsqlClient, epub_path: Path) -> None:
        ...
        if part_paths is None:
            self.logger.info(
                f"{epub_path.name}: a part upload failed, skipping (retried next run)"
            )
            return
    ...
    def _upload_parts(
        self, data: bytes, parts: list, txt_key: bytes, prefix: str
    ) -> list | None:
    ...
    def _write_bb_rows(
        self, epub_path: Path, txt_key: bytes, prefix: str, part_paths: list
    ) -> None:
    ...
    def _write_txt_meta(self, txt_id: int, epub_path: Path) -> None:
        ...
        self.bb.execute(
            "INSERT INTO txt_meta (txt_id, metadata) VALUES (?, ?)",
            [txt_id, compressed],
        )
    ...
    def _bundle_stale(
        self, aa: LibsqlClient, built_at_version: int, live_page_count: int
    ) -> bool:
        rows = aa.query(
    ...
    def _build_and_write_bundle(self, aa: LibsqlClient, current: list) -> None:
        ...
        if current:
            aa.execute(
                ...
                [int(time.time() * 1000), current[0][0]],
            )
    ...
    def _existing_or_new_index_keys(self, rows: list) -> tuple:
        if rows:
            return self.blob.decrypt(rows[0][0], self.umk).decode(), self.blob.decrypt(
                rows[0][1], self.umk
            )
