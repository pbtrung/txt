...
export class AdminInitializer {
  ...
  private requireR2Config(): R2ConfigResolved {
    if (!this.creds.r2Config) {
      throw new Error(
        "creds.json is missing r2_config -- --init-admin needs a real one to provision the admin's own database",
      );
    }
  ...
  private async commitInitialPages(
    ...
    const r2 = new R2Client(this.requireR2Config(), false, this.log);
