...
export function UnlockScreen() {
  ...
  return (
    <div>
      <h1>Unlock</h1>
      <button type="button" onClick={() => inputRef.current?.click()} disabled={unlocking}>
        {unlocking ? "Unlocking…" : "Choose File"}
      </button>
      <input ref={inputRef} type="file" accept="application/json" onChange={handleFileChange} hidden />
      {progress && (
        <p role="status">
