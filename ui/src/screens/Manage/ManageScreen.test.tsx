...
describe("ManageScreen", () => {
  ...
  describe("Users", () => {
    ...
    it("creates a user via the Create panel and reloads the list", async () => {
      ...
      // longer typed in at all; they're generated by createUser itself.
      ...
      // The modal now shows the generated credential JSON and can't be
      // closed except via this explicit confirmation -- the list only
      // reloads once the admin has acknowledged saving it.
      await waitFor(() => expect(screen.getByText(/"username": "carol"/)).toBeInTheDocument());
      await userEvent.click(screen.getByRole("button", { name: "I've saved this -- close" }));
