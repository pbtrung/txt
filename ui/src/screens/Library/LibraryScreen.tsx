      ...
      <div className="flex-grow-1 overflow-auto">
        ...
        <div className="list-group list-group-flush">
          ...
          <NavItem
            ...
            onClick={() =>
              selectView({ kind: "browse", dimension: "publisher" })
            }
          />
        </div>
      </div>
...
export function LibraryScreen() {
  ...
  const authorEntries = useMemo(
    () => browseEntries(books ?? [], "author"),
    [books],
  );
  const subjectEntries = useMemo(
    () => browseEntries(books ?? [], "subject"),
    [books],
  );
  const publisherEntries = useMemo(
    () => browseEntries(books ?? [], "publisher"),
    [books],
  );
  ...
  const metadataById = useMemo(
    () => new Map((books ?? []).map((b) => [b.txtId, b.info])),
    [books],
  );
      ...
      <div className="flex-grow-1 d-flex flex-column flex-lg-row overflow-hidden">
        ...
        {refreshing ? (
          ...
          >
            <ProgressStatus
              progress={progress}
              fallbackLabel="Refreshing your library"
            />
          </div>
        ) : (
          <>
            ...
            >
              ...
              <div className="flex-grow-1 d-flex flex-column overflow-hidden">
                {loading && (
                  <p className="text-body-secondary p-3">
                    ...
                    renderRow={(entry) => (
                      <button
                        ...
                        onClick={() =>
                          selectView({
                            kind: "browseValue",
                            dimension: (view as { dimension: BrowseDimension })
                              .dimension,
                            value: entry.value,
                          })
                        }
                ...
                {!loading && view.kind !== "recent" && bookList && (
                  <VirtualizedListGroup
                    ...
                    renderRow={(book) => (
                      <BookRow book={book} onClick={() => openBook(book)} />
                    )}
                  />
                )}
              </div>
