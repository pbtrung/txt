...
const LIBRARY: LibraryBook[] = [
  book({ txtId: 1, title: "Dune", authors: ["Frank Herbert"], subjects: ["Science Fiction"], publisher: "Ace" }),
  book({ txtId: 2, title: "A Wizard of Earthsea", authors: ["Ursula K. Le Guin"], subjects: ["Fantasy"] }),
];

function renderScreen(books: LibraryBook[] | null) {
  const session = { libraryIndexBytes: new Uint8Array() } as VaultSession;
  vi.mocked(useVault).mockReturnValue({ status: "unlocked", session, error: null, progress: null, unlock: vi.fn(), lock: vi.fn() });
...
describe("LibraryScreen", () => {
  ...
  it("links each book to /read/:txtId", () => {
    renderScreen(LIBRARY);
    expect(screen.getByRole("link", { name: /Dune/ })).toHaveAttribute("href", "/read/1");
  });
});
