...
function baseResult(
  overrides: Partial<UseReaderBookResult> = {},
): UseReaderBookResult {
  return {
    loading: false,
    error: null,
...
async function openInfo() {
  await userEvent.click(
    screen.getByRole("button", { name: /about this book/i }),
  );
}
...
describe("ReaderScreen", () => {
  ...
  it("uses small (btn-sm) Previous/Next buttons, matching the bottom bar's other controls", () => {
    renderReader(baseResult());
  ...
  describe("font size", () => {
    ...
    it("sits to the left of the Previous button in the bottom bar", () => {
      ...
      expect(
        select.compareDocumentPosition(previous) &
  ...
  describe("editable part number", () => {
    it("shows the current part number and total", () => {
      renderReader(baseResult());
      expect(screen.getByRole("textbox", { name: /go to part/i })).toHaveValue(
        "14",
      );
  ...
  describe("About this book dropdown", () => {
    ...
    it("truncates a long description to 200 characters with a Show more button", async () => {
      ...
      renderReader(
        baseResult({
          info: { ...baseResult().info!, description: longDescription },
        }),
      );
      await openInfo();
    ...
    it("hides the 'All metadata' section entirely when there's none to show", async () => {
      renderReader(
        baseResult({ info: { ...baseResult().info!, rawMetadata: [] } }),
      );
      await openInfo();
  ...
  describe("Bookmarks dropdown", () => {
    ...
    it("shows the filled icon when the book has bookmarks, outline otherwise", () => {
      ...
      vi.mocked(useReaderBookModule.useReaderBook).mockReturnValue(
        baseResult({ bookmarks: [] }),
      );
      rerender(
        <MemoryRouter initialEntries={["/read/1"]}>
          <Routes>
      ...
      expect(
        document.querySelector(".bi-bookmark-fill"),
      ).not.toBeInTheDocument();
    });
    ...
    it("opens on click, showing part/line and a text preview", async () => {
      ...
      expect(
        screen.getByText("“First paragraph of part 14.”"),
      ).toBeInTheDocument();
      expect(screen.getByText("Part 8 · Line 2")).toBeInTheDocument();
      expect(
        screen.getByText("“Some earlier line preview”"),
      ).toBeInTheDocument();
    });
  ...
  it("bookmarks a specific line via that line's own gutter button", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /bookmark line 2/i }),
    );
  ...
  it("marks an already-bookmarked line's gutter icon as pressed", () => {
    renderReader(baseResult());
    expect(
      screen.getByRole("button", { name: /bookmark line 1/i }),
