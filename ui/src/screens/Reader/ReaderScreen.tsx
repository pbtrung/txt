...
export function ReaderScreen() {
  ...
  const bookmarkedLines = useMemo(
    () =>
      new Set(
        bookmarks
          .filter((b) => b.partNum === currentPartNum)
          .map((b) => b.line),
      ),
  ...
  const descriptionIsLong =
    (descriptionPlain?.length ?? 0) > DESCRIPTION_PREVIEW_LEN;
    ...
    <div className="shell-60 d-flex flex-column vh-100">
      <div className="border-bottom d-flex align-items-center gap-3 ps-2 ps-sm-3 pe-3 py-2">
        ...
        <div className="flex-grow-1 text-truncate">
          ...
          {info?.author && (
            <div className="text-body-secondary small text-truncate d-sm-none">
              {info.author}
            </div>
          )}
        </div>
          ...
          {infoMenu.open && (
            ...
            >
              ...
              {/* The curated fields above (title/author/series/subjects/
                  ...
                  field this book's full metadata carries, verbatim key and
                  values. */}
