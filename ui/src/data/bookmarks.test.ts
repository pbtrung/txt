...
beforeEach(async () => {
  ...
  db = await SqliteDb.open(`/bookmarks-test-${dbCounter++}.db`, {
    rawKey: rootKey,
  });
  db.exec(`
...
// loadBookmarksMap is the only reader dbWorker.ts actually calls (one
// whole-vault query, not one per document -- see its own doc comment), so
// it stands in here as the assertion vehicle for addBookmark/removeBookmark
// behavior too, rather than keeping a separate per-document reader around
// solely for tests to call.
function listFor(txtId: number): bookmarks.Bookmark[] {
  return bookmarks.loadBookmarksMap(db).get(txtId) ?? [];
}

describe("addBookmark / loadBookmarksMap", () => {
  ...
  it("groups bookmarks by txt_id", () => {
    ...
    expect(listFor(1)).toHaveLength(1);
    expect(listFor(2)).toHaveLength(1);
  });
});
...
describe("removeBookmark", () => {
  it("removes a bookmark by id", () => {
    ...
    const id = listFor(1)[0]!.id;
    bookmarks.removeBookmark(db, id);
    expect(listFor(1)).toEqual([]);
  });
});
