...
export class RqliteHttpClient {
  ...
  async commit(
    ...
    if (!res.ok)
      throw new Error(`rqlite /db/execute COMMIT failed: HTTP ${res.status}`);
  ...
  private async parseResponse(
    ...
    if (!res.ok)
      throw new Error(
        `rqlite /db/${endpoint} ${statementId} failed: HTTP ${res.status}`,
      );
    const decoded = (await res.json()) as { results?: RqliteResult[] };
    if (!decoded.results)
      throw new Error(
        `rqlite /db/${endpoint} ${statementId}: malformed response (no results)`,
      );
...
function checkCommitResults(
  ...
  if (casUpdate.error)
    throw new Error(`COMMIT CAS update failed: ${casUpdate.error}`);
  const casWon = (casUpdate.rows_affected ?? 0) > 0;
  if (!casWon) return false;
  if (insert.error)
    throw new Error(`COMMIT page insert failed: ${insert.error}`);
...
function shortResultsError(results: RqliteResult[]): Error {
  const insert = results[0];
  if (insert?.error)
    return new Error(`COMMIT page insert failed: ${insert.error}`);
  return new Error(
...
export async function resolveTargetDbId(
  ...
  const batch = [
    {
      sql: "SELECT user_id FROM api_keys WHERE key_hash = ?",
