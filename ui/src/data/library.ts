...
export async function loadLibrary(db: SqliteDb): Promise<LibrarySnapshot> {
  const stmt = db.prepare("SELECT id, name, metadata, last_part_num, last_accessed FROM txt;");
  ...
  while (stmt.step()) {
    ...
    const opf = await parseMetadataBlob(stmt.columnIsNull(2) ? null : stmt.columnBlob(2));
