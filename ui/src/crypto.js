...
function _aeadDoEncrypt(ctx, key, iv, pt, aad) {
  const keyPtr = alloc(key), ivPtr = alloc(iv);
  const r = lc._lc_aead_setkey(ctx, keyPtr, key.length, ivPtr, iv.length);
  lc._free(keyPtr); lc._free(ivPtr);
  if (r !== 0) throw new Error('lc_aead_setkey failed');
  const ptPtr = alloc(pt), ctPtr = lc._malloc(pt.length);
  const aadPtr = alloc(aad), tagPtr = lc._malloc(TAG_LEN);
  const enc = lc._lc_aead_encrypt(ctx, ptPtr, ctPtr, pt.length, aadPtr, aad.length, tagPtr, TAG_LEN);
  ...
  out.set(ctBytes); out.set(tagBytes, ctBytes.length);
  return out;
}
...
function _aeadEncrypt(key, iv, pt, aad) {
  ...
  try {
    ...
    return _aeadDoEncrypt(ctx, key, iv, pt, aad);
  } finally {
...
function _aeadDoDecrypt(ctx, key, iv, ctTag, aad) {
  ...
  lc._free(keyPtr); lc._free(ivPtr);
  if (r !== 0) throw new Error('lc_aead_setkey failed');
  const ct = ctTag.slice(0, -TAG_LEN), tag = ctTag.slice(-TAG_LEN);
  const ctPtr = alloc(ct), ptPtr = lc._malloc(ct.length);
  const aadPtr = alloc(aad), tagPtr = alloc(tag);
  const dec = lc._lc_aead_decrypt(ctx, ctPtr, ptPtr, ct.length, aadPtr, aad.length, tagPtr, TAG_LEN);
  const out = lc.HEAPU8.slice(ptPtr, ptPtr + ct.length);
  lc._free(ctPtr); lc._free(ptPtr); lc._free(aadPtr); lc._free(tagPtr);
...
function _aeadDecrypt(key, iv, ctTag, aad) {
  ...
  try {
    ...
    if (ok !== 0) throw new Error('lc_ak_alloc_taglen failed');
    ctx = lc.HEAP32[ctxSlot >> 2];
    return _aeadDoDecrypt(ctx, key, iv, ctTag, aad);
  } finally {
    if (ctx) lc._lc_aead_zero_free(ctx);
    lc._free(ctxSlot);
  }
...
export function encryptBookmark(obj, masterKey) {
  ...
  out.set(salt); out.set(ctTag, SALT_LEN);
  return out;
}
