...
describe("blob", () => {
  ...
  it("rejects a blob shorter than the minimum length", async () => {
    await expect(
      blob.decrypt(new Uint8Array(64), new Uint8Array(10), false),
    ).rejects.toThrow("blob shorter than minimum valid length");
  });
  ...
  it("rejects a blob with bad magic bytes", async () => {
    ...
    const encoded = await blob.encrypt(ikm, new TextEncoder().encode("secret"));
    encoded[0] ^= 0xff;
    await expect(blob.decrypt(ikm, encoded, false)).rejects.toThrow(
      "bad magic",
    );
  });
});
