...
export async function encrypt(plaintext: Uint8Array, ikm: Uint8Array): Promise<Uint8Array> {
...
function parseHeader(blob: Uint8Array): { salt: Uint8Array; ciphertext: Uint8Array; tag: Uint8Array } {
  ...
  return { salt: blob.slice(4, 68), ciphertext: blob.slice(68, blob.length - 64), tag: blob.slice(blob.length - 64) };
}
