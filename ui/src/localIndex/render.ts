...
function extractEntryPaths(indexHtmlText: string): EntryPaths {
  ...
  if (!script) {
    throw new RenderError(
      'index.html has no <script type="module" src=...> to load',
    );
  }
...
export function renderApp(
  assetBaseUrl: string,
  verified: Map<string, Uint8Array>,
): void {
  const indexHtmlBytes = verified.get("index.html");
  if (!indexHtmlBytes) {
    throw new RenderError("manifest.json didn't include index.html");
  }
  const { jsPath, cssPath } = extractEntryPaths(
    new TextDecoder().decode(indexHtmlBytes),
  );
  verbose(
    `localIndex: entry paths -- js=${jsPath}, css=${cssPath ?? "(none)"}`,
  );

  const jsBytes = verified.get(jsPath);
  if (!jsBytes) {
    throw new RenderError(
      `index.html references ${jsPath}, which manifest.json didn't include`,
    );
  }
