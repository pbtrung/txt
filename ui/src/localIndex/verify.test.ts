...
const INDEX_HTML = new TextEncoder().encode(
...
describe("verifyAssets", () => {
  ...
  it("verifies a correctly signed manifest and every listed asset", async () => {
    ...
    const verified = await verifyAssets(ASSET_BASE_URL, publicKey, (step) =>
      progress.push(step),
    );

    expect(Array.from(verified.get("index.html")!)).toEqual(
      Array.from(INDEX_HTML),
    );
    expect(Array.from(verified.get("assets/app.js")!)).toEqual(
      Array.from(APP_JS),
    );
  ...
  it("rejects a manifest whose signature doesn't verify", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      fakeFetch({ "manifest.json": manifestBytes, "manifest.sig": sig }),
    );

    await expect(
      verifyAssets(ASSET_BASE_URL, wrongPublicKey, () => {}),
  ...
  it("rejects a manifest signed correctly but tampered with after signing", async () => {
    ...
    const tampered = new TextEncoder().encode(
      JSON.stringify({ "index.html": "not-a-real-hash" }),
    );
    vi.stubGlobal(
      "fetch",
      fakeFetch({ "manifest.json": tampered, "manifest.sig": sig }),
    );

    await expect(
      verifyAssets(ASSET_BASE_URL, publicKey, () => {}),
    ).rejects.toThrow(/signature check/);
  });
  ...
  it("wraps a bare network-level fetch failure with a plain message when not on file://", async () => {
    ...
    await expect(
      verifyAssets(ASSET_BASE_URL, publicKey, () => {}),
    ).rejects.toThrow(/failed to fetch .*: Failed to fetch/);
    await expect(
      verifyAssets(ASSET_BASE_URL, publicKey, () => {}),
    ).rejects.not.toThrow(/file:\/\//);
  });
  ...
  it("rejects a manifest that lists no files", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      fakeFetch({ "manifest.json": emptyManifestBytes, "manifest.sig": sig }),
    );
