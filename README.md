...
- **`txt.ts`**, the admin CLI — brings an existing vault onto this schema (`--migrate`), then maintains it day to day locally (`--clean-bucket`, `--collect-garbage`, `--vacuum`) or against a live deployment over the network (`--remote-vacuum`), and measures/exercises the lazy-remote-read/write path (`--test-perf`, `--test-write`). See [docs/cli.md](docs/cli.md) for the full command reference.
