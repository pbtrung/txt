...
- `--no-delete` to keep old R2/S3 objects around instead of deleting them once their replacement is confirmed written.
- `--verbose` for per-document progress.
- Resumable within a single run: a part already present in the in-progress output is skipped rather than reprocessed.
...
- `--verbose` — prints per-document progress instead of only a final summary.
...
`txt/migrate.test.ts` runs the whole migration end to end against a small synthetic vault and a local mock R2 server — it never touches a real database or bucket.
