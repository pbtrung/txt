...
`creds.json` fields: `turso_org_token`, `turso_ctl_db_url`, `turso_group`, `turso_org`, `firebase_email`, `firebase_password`, `firebase_api_key`, and optionally `display_name` and `user_root_key` (generated and written back on first use if left empty). `--ingest` additionally needs `r2_config`: `endpoint`, `read_only_access_key_id`, `read_only_secret_access_key`, `read_write_access_key_id`, `read_write_secret_access_key`, `region`, `bucket`.
