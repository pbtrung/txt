...
- `txt/` — one module per concern:
  ...
  - `sqlite_engine.py` — real SQLCipher read/write against `sqlcipher.wasm`, used only by `db_updater.py` (below) — an in-memory `sqlite3_vfs` (function pointers installed into the wasm indirect function table via `sqlite3_js_vfs_register`) backs a whole database in a Python `bytearray`, since this build has no working native filesystem VFS and `:memory:` connections never engage the codec. Keys are 256–8192 raw bytes (`sqlite3_key`), not a passphrase.
  ...
  - `db_updater.py` — `--update-db`: still targets the design's predecessor (rqlite, a whole downloaded SQLCipher database file) and isn't reachable from `cli.py` until it's rewritten for the D1 design (`docs/milestones.md` Milestone 9); its test skips itself at import time until then.
