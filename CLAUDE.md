...
- Running the mounted app under `local_index.html`'s opaque/null origin needs one accommodation elsewhere in `ui/src/`: `appRouter.ts`'s `pickRouterComponent()` swaps `BrowserRouter` for `MemoryRouter` when `location.origin === "null"` (checked via the origin, not `location.protocol === "file:"` — Android in particular commonly opens a local file via a `content://` URI instead, just as opaque as `file://` but missed by a protocol-based check), since the History API's `pushState`/`replaceState` (which `BrowserRouter` needs) throws unconditionally in a null-origin document.
