...
`ui/` is a second, browser-based client implementing [docs/ui.md](docs/ui.md)'s design (Vue 3 + TypeScript + Vite + Vitest, Bootstrap CSS/Icons): Unlock, Library, and Reader screens over the same vault, reading (and lightly writing: read-position, bookmarks) it directly from the browser rather than through `txt.py`. Built with the Composition API (`<script setup lang="ts">` SFCs) throughout, `vue-router` for routing, and `@testing-library/vue` for component tests — originally built in React, migrated to Vue after a React 19 fiber-reconciler edge case (`commitDeletionEffectsOnFiber`'s `hostParent` bookkeeping, surfaced as a "Failed to execute 'removeChild' on 'Node'" crash during an error-boundary recovery commit) had no confirmed upstream fix; Vue's reactivity/patching model has no equivalent to React's Fiber architecture, so this specific bug class doesn't apply there.
...
- `ui/src/state/vault.ts`'s `useVault()` — a plain module-level singleton composable, not a Vue provide/inject context, since there's never more than one vault instance and (unlike a context) it needs to be readable from `router.ts`'s navigation guard too, which runs outside any component's `setup()` — holds the unlocked session in memory only, never persisted, so a reload always lands back on Unlock.
