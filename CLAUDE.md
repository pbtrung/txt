...
This repo is the **TypeScript CLI** (`txt.ts`) for administering that system — not the end-user app itself (no UI here). It's also mid-migration: an earlier version of this project ran on Turso/rqlite with content chunked directly into R2 (schema as of commit `1ed39d433365c39a6973303c171c7bb5510d7e3e`); `txt/owner.ts` and `--migrate` still read that old schema to pull data across.
...
- `txt/owner.ts` — `TxtOwner`: reads the **legacy** (pre-InstantDB) schema — user/key resolution, known R2 paths, part/metadata decoding. Used by `--clean-bucket` and the "from" side of `--migrate`.
...
- `txt/creds.ts` / `txt/initAdminCreds.ts` — creds.json loaders. `Creds` (creds.ts) is the legacy-account shape (`--clean-bucket`, `--migrate --from-creds`); `InitAdminCreds` (initAdminCreds.ts) is the live-account shape (`--init-admin`, `--migrate --to-creds`).
...
- **InstaQL always returns a linked sub-entity as an array**, regardless of that link's own declared cardinality (`has: "one"` included). `row.pointerFile[0].url`, not `row.pointerFile.url`. Got this wrong once already for `users.dbMeta` (silent `undefined` fields → a 0-byte "reopened" database that looked brand new to SQLite, no error until the first real query). See the `instantdb-instaql-array-links` memory.
