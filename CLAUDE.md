...
A fully client-side-encrypted text vault. Backend: Turso (libSQL/SQLite-compatible cloud) holds all structured data — accounts, wrapped keys, document content, sharing grants, bookmarks, and read-position — but only ever sees ciphertext, or values that are already public (public keys, KEM ciphertexts, encapsulation ephemerals). All encryption, decryption, and key (un)wrapping happens client-side (or in admin tooling), never in the database.

Each user has an account (`users`, looked up by a keyed-HMAC username hash, authenticated by a PBKDF2 password check), a master key (`umk`, wrapped under a per-user root key held outside Turso in per-user JSON config), and a composite ML-KEM-1024 + X448 keypair (`key_store`) used to receive documents shared by other users. A user's documents (`txt`) are chunked into parts (`txt_parts`), each document with its own `txt_key` wrapped under the owner's `umk`. A document can also be shared with another user (`txt_shares`), which re-wraps that same `txt_key` under the recipient's public key instead of revealing the owner's `umk`.
