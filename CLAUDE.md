...
- Running the mounted app under `local_index.html`'s opaque/null origin needs one accommodation elsewhere in `ui/src/`: `appRouter.ts`'s `pickRouterComponent()` swaps `BrowserRouter` for `MemoryRouter` since the History API's `pushState`/`replaceState` (which `BrowserRouter` needs) throws unconditionally in a null-origin document — detected empirically (a no-op `replaceState()` attempted in a try/catch) rather than by inspecting `location.protocol`/`location.origin`, since neither string reliably identifies every opaque-origin case in practice (`protocol === "file:"` misses Android's `content://` URIs; `origin === "null"` didn't catch those either, confirmed on a real device). The probe URL matters too: it has to be path-absolute (`location.pathname` + `location.search` + `location.hash`), not the full `location.href` — an absolute URL bypasses `document.baseURI` entirely, so it can't reproduce the mismatch a real (relative) navigation would hit once `render.ts` has pointed `<base>` at `asset_base_url`.
