...
- **Run `npx prettier --write <files>` before committing** anything under `txt/` or the root config files. `sqlcipher/` and `docs/` are intentionally excluded (see `.prettierignore`) — don't reformat them.
