...
- `txt/` — one module per concern:
  ...
  - `turso_api.py` and `libsql_client.py` — source-control adapters used to read and validate a Turso owner during `--migrate` and by the remaining source maintenance commands.
  ...
  - `owner_init.py` — `OwnerInitializer`, the idempotent `--init-owner` implementation. It installs an absent rqlite schema, creates exactly one `owner_control` row, and validates existing owner material.
  ...
  - `account_session.py` — decrypts the source control payload used by existing Python ingestion maintenance.
  ...
  - `ctl_updater.py` — source control-schema validation retained for migration preparation; it supports a full `--dry-run` preview.
  - `db_updater.py` — `DbUpdater`, the `--update-db` command: migrates and validates the complete catalog, CFI reading-state, bookmark, share, and named-migration schema. R2 is always the input; local files are inspection checkpoints only.
...
- Reuse the existing generic pieces (`RqliteClient`, `LibsqlClient`, `TursoClient`, `CryptoBlob`, `FirebaseAuth`, `AccountSession`, `R2Client`, `SqliteEngine`) instead of duplicating HTTP, crypto, or storage logic in a new command.
