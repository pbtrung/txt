...
- Running the mounted app under `local_index.html`'s `file://` (an opaque/null origin) needs two accommodations elsewhere in `ui/src/`, both keyed off `location.protocol === "file:"`: `appRouter.ts`'s `pickRouterComponent()` swaps `BrowserRouter` for `MemoryRouter` (the History API's `pushState`/`replaceState`, which `BrowserRouter` needs, throws unconditionally in a null-origin document), and `data/r2.ts`'s `getObject()` (`shouldPresign()`) switches from a header-signed R2 GET to a short-lived (60s) query-string-signed one (a header-signed GET is non-simple and needs a CORS preflight R2 can't answer for a null origin; a query-signed one carries no custom headers, so it's simple and skips the preflight entirely).
