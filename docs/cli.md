...
`PRAGMA incremental_vacuum(N)` sidesteps needing any of that: it only reclaims up to `N` pages (`commands.ts`'s `INCREMENTAL_VACUUM_PAGE_COUNT`, currently 50 — picked with margin under `client_body_buffer_size` even accounting for a reclaimed page touching a few more, e.g. its pointer-map entry and b-tree parent, not a strict 1:1 ratio), so each call is already its own small, complete, one-request commit — no batching protocol needed. It's a harmless no-op, not an error, on a vault that hasn't been converted via `--convert-auto-vacuum` yet: `--remote-vacuum` checks `PRAGMA auto_vacuum` explicitly first and logs precisely which of the two no-op cases applies (`auto_vacuum` isn't `INCREMENTAL` yet, vs. it already is but there's nothing free to reclaim) rather than one ambiguous message covering both — useful for confirming whether a `--convert-auto-vacuum`'d database actually made it onto this deployment (see that command's own section: it only writes a local `--out-db` file, getting that onto a live deployment is a separate, manual step, e.g. rqlite's own `.restore` console command). `commands.ts`'s `prefetchAllPages`/`remoteVfs.ts`'s `primeCache` (the batched-prefetch machinery a full user-database `VACUUM` step would need) are still in place, just not wired into `RemoteVacuumCommand` -- `incremental_vacuum` only ever touches a small, bounded set of pages, so there's nothing to prefetch for.
