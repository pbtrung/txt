...
6. A second `cred_store` row for the administrator's own backup: `owner_id` = the administrator's own uid, `for_user_id` = the new user's uid, the same `{ display_name, db_master_key, db_path, db_prefix }` payload re-wrapped under the administrator's own `umk` this time. This is what lets `--update-db` (docs/data_model.md) reach every user's database from just the administrator's own creds.json — it only covers accounts provisioned from here on; anyone provisioned before this step existed has no backup row and still needs their own creds.json for anything that needs `db_master_key`.
