...
- **Account footer**: pinned to the bottom of the left nav (both the lg+ sidebar and the below-`lg` dropdown share this content) — a person icon and the signed-in `display_name` on the left, two icon-only buttons (border tinted the brass accent, same as the drawer toggle) on the right: Refresh, then Lock. This is the only place either action lives; there's no separate button for them in the top bar. Refresh re-loads metadata/access/bookmarks from Turso/R2 without a full re-unlock (for a book ingested/shared, or a bookmark/read-position added, from elsewhere since unlocking) — the search field disables for its duration. At lg+, the entire persistent left pane below the wordmark (nav items, account footer, Lock/Refresh included) and the content pane alike collapse into a single centered spinner while this runs; only the top bar survives untouched. Below lg, there's no persistent left pane to replace this way (it's a dropdown instead, off the top bar) — that copy of the nav keeps working as usual, its own Refresh button swapping to a spinner and disabling itself for the duration exactly as it always did, and its drawer toggle (the book icon beside the wordmark) disables right along with it, consistent with the rest — it just can't newly open mid-refresh; one already open when refreshing starts stays open and usable.
