...
- **Info dropdown**: title, author, series if any, subject tags, and a short description pulled from the book's catalog metadata. A description longer than 200 characters shows truncated with an ellipsis and a "Show more"/"Show less" toggle rather than the full text up front. Below that curated summary, an "All metadata" section lists every OPF/Calibre field the catalog entry actually carries — verbatim field name and value(s), one row each — so nothing in the underlying record is hidden even if it isn't one of the fields surfaced above; omitted entirely for a book with no metadata beyond its filename.
