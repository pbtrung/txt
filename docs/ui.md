...
- **Info dropdown**: title, author, series if any, subject tags, and a short description pulled from the book's catalog metadata. A description longer than 200 characters shows truncated with an ellipsis and a "Show more"/"Show less" toggle rather than the full text up front. Below that curated summary, an "All metadata" section lists every OPF/Calibre field the catalog entry actually carries — one row each — so nothing in the underlying record is hidden even if it isn't one of the fields surfaced above; omitted entirely for a book with no metadata beyond its filename. Calibre's own bookkeeping fields (`calibre:rating`, `calibre:title_sort`) are dropped rather than shown, `calibre:timestamp` is relabeled `timestamp`, and both it and `date` are reformatted for a human reader ("January 15, 2020", or "January 15, 2020, 8:23 AM" if the timestamp actually carries a non-midnight time) instead of showing the raw ISO string.
