...
`ui/src/data/r2Config.ts`/`r2.ts` are hard-restricted to read-only for every browser session, admin included — there's no `r2_config` field on the web `Creds` shape at all (unlike the CLI's `AdminCreds`/`UserCreds`, which always carry one directly), and every account's row in Turso's own `r2_config` table only ever holds the read-only key pair (`txt/admin.py`'s `_ensure_r2_config`: "Only the read-only key pair is ever persisted to Turso, regardless of role"). The Manage screen's admin actions are designed around that constraint, not against it:
