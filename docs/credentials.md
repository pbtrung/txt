...
## The Manage screen stays R2 read-only, even for the admin role

`ui/src/data/r2Config.ts`/`r2.ts` are hard-restricted to read-only for every browser session, admin included — there's no `r2_config` field on the web `Creds` shape at all (unlike the CLI's `AdminCreds`/`UserCreds`, which always carry one directly). Every account's row in Turso's own `r2_config` table only ever holds the read-only key pair when first created (`txt/admin.py`'s `_ensure_r2_config`), with one opt-in exception: `txt.py --update-r2-config` (see [cli.md](cli.md)) can add the admin's own read-write pair into their own row afterward. **`ui/src/data/r2Config.ts`'s `parseR2Config` doesn't know about that exception yet** — it unconditionally throws if it sees `read_write_access_key_id`/`read_write_secret_access_key` in a row's decrypted config, admin or not — so running `--update-r2-config` today breaks that same admin's own web UI unlock (`fetchR2Config` would throw) until/unless `parseR2Config` is updated to tolerate it for an admin session. The Manage screen's admin actions, as they exist today, are designed around the *original* constraint (no read-write keys anywhere in Turso, ever), not this newer opt-in:

- **Deleting a txt or a whole user account is Turso-only.** Rows are removed, but the R2 objects they pointed at (part bodies, and a deleted account's last `txt_metadata` object) are left as orphans — the Manage screen says so explicitly wherever this applies. `txt.py --txt-clean-bucket`, run separately with real read-write R2 keys, is what actually reclaims that storage; this is exactly the tool it already exists for.
- **Editing a txt's metadata isn't offered in the browser at all.** Unlike a delete, there's no orphan to defer here — `txt_metadata`'s content lives in one R2 object per account, so changing it means overwriting that object in place, an R2 PUT with no CLI-deferred equivalent. Kept out of scope entirely rather than granting the admin's web session a narrow read-write R2 credential just for this one case; metadata edits stay a `--txt-ingest`/OPF-sidecar operation.
