...
- `asset_sign_key`, `asset_hashes` — used only by the web UI (`ui/src/data/creds.ts`'s `parseCreds`), not by `txt/creds.py` (which reads specific known keys and ignores the rest, so their presence here needs no schema change on this side). Colocating them in the same file `txt.py` reads is a convenience — one credential file doubles as both this role's `Creds` input and the UI's own unlock config — not a requirement either side imposes on the other. See the root [README.md](../README.md#asset-integrity-check-asset_sign_keyasset_hashes) for what they're for and how `ui/scripts/sign-assets.mjs`/`ui/src/integrity/verifyAssets.ts` use them.
