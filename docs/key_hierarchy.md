...
A document mid `txt.ts --rekey-doc` (docs/protocols.md's Hard revocation (rekey) protocol) carries a second, parallel key chain, rooted the same way as the first:
