...
CREATE TABLE IF NOT EXISTS txt_parts (
    ...
    path     BLOB    NOT NULL        -- magic||version||salt||Ascon-Keccak(path)||tag, wrapped under this txt's txt_key; path is the R2 object key holding this part's actual content
);
...
CREATE TABLE IF NOT EXISTS txt_shares (
    ...
    txt_key       BLOB    NOT NULL,   -- same txt_key bytes as txt.txt_key, wrapped for this recipient via HKDF-SHA3-512(IKM=ss, salt) -> 128-byte OKM (see crypto.md Encapsulate/Decapsulate); ss is the shared secret from lc_kyber_1024_x448_enc_kdf/_dec_kdf, never stored
...
CREATE TABLE IF NOT EXISTS txt_access (
    txt_id        INTEGER NOT NULL REFERENCES txt(id) ON DELETE CASCADE,
    user_id       INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    access BLOB NOT NULL, -- magic||version||salt||Ascon-Keccak(brotli(JSON))||tag, wrapped under this txt's txt_key; JSON = {"last_part_num": int, "last_accessed": int (unix ms)}
...
- **`txt_shares`** — one row per (document, recipient) share grant. Carries the same `txt_key` bytes as the owning `txt` row, but wrapped asymmetrically under the recipient's `key_store.pub_key` (crypto.md's Encapsulate/Decapsulate) instead of the owner's `umk`, so the recipient can decrypt `txt_parts`/`txt_access`/`bookmarks` for that document using their own `key_store.priv_key` without ever learning the owner's `umk` or `txt_key`-wrapping secret. `salt_kem_ct` is `salt || lc_kyber_1024_x448_ct`, the public value the recipient needs to Decapsulate before they can unwrap `txt_key` — it isn't sensitive on its own (an attacker with only `salt_kem_ct` can't recover `txt_key` without the recipient's `priv_key`, since the wrapping OKM is derived from the shared secret `ss`, not from `salt_kem_ct`).
...
- **`txt_access`** — per-(user, txt) read position, keyed by the pair so each user (owner or share recipient) tracks their own progress through a document independently. `access` is an encrypted JSON blob, `{"last_part_num": int, "last_accessed": int}`, wrapped under that document's `txt_key` — same pattern as `bookmarks.bookmark`.
...
umk  (umk_store.umk — 64 random bytes, generated once per user)
    ...
    │        └──▶ txt_parts.path, txt_access.access, bookmarks.bookmark  (per document's part paths, read position, and bookmarks)
    │
    ...
    └──▶ key_store.{pub_key, priv_key}   (lc_kyber_1024_x448 composite keypair — per user;
        ...
        txt_shares.{salt_kem_ct, txt_key}   (per (document, recipient) —
             ...
             │  unwraps txt_key, then reads txt_parts.path/txt_access.access/
             │  bookmarks.bookmark same as the owner
             ▼
...
- `umk` is 64 random bytes generated once when a user's `umk_store` row is created. Unwrapped, it is used directly as HKDF IKM (no intermediate per-purpose derivation) to wrap and unwrap `txt.txt_key`, `txt_metadata.txt_metadata_key`, `key_store.priv_key`, and `r2_config.config`.
- `txt_key` and `txt_metadata_key` are each 64 random bytes, same size as `umk`. Unwrapped, they're themselves used directly as IKM to encrypt/decrypt the content columns they protect. `txt_key` — the document's own (unwrapped) key — is the IKM for `txt_parts.path`, `txt_access.access`, and `bookmarks.bookmark`, since all three are scoped per-`txt_id`. `txt_metadata_key` is the IKM for `txt_metadata.content`.
...
- `txt_shares` lets a document owner grant another user access without either party learning the other's `umk`: the owner generates a random 64-byte `salt` and Encapsulates (crypto.md) against the recipient's `key_store.pub_key` via `lc_kyber_1024_x448_enc_kdf`, producing a KEM ciphertext `ct` and shared secret `ss`. It stores `salt_kem_ct = salt || ct` (public) and wraps `txt_key` using `HKDF-SHA3-512(IKM=ss, salt)` → 128-byte OKM, same AEAD-key/IV split as a standard Encrypt. The recipient later parses `salt` and `ct` back out of `salt_kem_ct`, Decapsulates `ct` with their own `priv_key` via `lc_kyber_1024_x448_dec_kdf` to recover `ss` (never stored or transmitted), and reruns the same HKDF derivation to unwrap `txt_key`.
...
- **Composite KEM combiner binding.** `ss` comes straight out of leancrypto's `lc_kyber_1024_x448_enc_kdf`/`_dec_kdf` — the ML-KEM-1024/X448 combining happens inside leancrypto, opaque to this schema, so there's no app-level combiner step to reason about here. Whether that internal combiner binds `ct` or either party's public key into `ss` (the way X-Wing folds `ct`/public keys into its derivation for domain separation) is a leancrypto implementation detail worth confirming separately, not something this data model controls.
