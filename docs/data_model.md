...
- **`bookmarks`** — per-(user, txt) bookmark list, for owners and share recipients alike. `bookmark` is an encrypted JSON blob, `{"part_num": int, "line": int, "txt_preview": str}` (`line` is 1-based, indexing into that part's text as split into lines the same way the reader renders them; `txt_preview` is that line's text truncated to 60 characters), wrapped under that document's `txt_key` — same pattern as `txt_access.access`. Unlike `txt_access`, a bookmark carries no timestamp of its own — `bookmarks.id` (autoincrement) is recency order. The FIFO trigger caps each user at 20 bookmarks per document by evicting the oldest (lowest `id`) on overflow, so storage stays bounded without an application-side cleanup step.
