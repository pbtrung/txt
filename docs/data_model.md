...
- **`txt_shares`** — one row per (document, recipient) share grant. Carries the same `txt_key` bytes as the owning `txt` row, but wrapped asymmetrically under the recipient's `key_store.pub_key` (crypto.md's Encapsulate/Decapsulate) instead of the owner's `umk`, so the recipient can decrypt `txt_parts` for that document using their own `key_store.priv_key` without ever learning the owner's `umk` or `txt_key`-wrapping secret. (The recipient's own read position and bookmarks for this document live under their own `umk` chain — `txt_access`/`bookmarks` — independent of this share; see below.) `salt_kem_ct` is `salt || lc_kyber_1024_x448_ct`, the public value the recipient needs to Decapsulate before they can unwrap `txt_key` — it isn't sensitive on its own (an attacker with only `salt_kem_ct` can't recover `txt_key` without the recipient's `priv_key`, since the wrapping OKM is derived from the shared secret `ss`, not from `salt_kem_ct`).
  ...
  Granting a share (`ui/src/data/adminShares.ts`'s `grantShare`) does one more thing than the `txt_shares` insert itself: it copies the owner's `txt_metadata` entry for this document into the *recipient's own* `txt_metadata.content` too, so the recipient's Library actually shows a name for it — no new column or KEM wrap needed for this, since the admin already holds an escrow path to any regular user's `umk` (`users.creds`, below) and `txt_metadata.content`/`persistMetadataContent` are already generic over any account's `userId`/`umk`, not hardcoded to the caller's own session. That copy write happens *before* the `txt_shares` insert, so a failure never leaves a dangling share grant. Revoking a share, or deleting the document entirely, scrubs that copy back out the same way (via the same escrow path) *before* removing the grant/document itself — consistently a hard failure on either side: recovering a recipient's umk failing means the whole grant/revoke/delete call fails outright, never leaving a stale (grantable-but-invisible, or revoked-but-still-listed) copy behind.
...
- **`txt_metadata`** — one row per user (`user_id` is `UNIQUE`), created by `--init` (`AdminInitializer._ensure_txt_metadata`) alongside `umk_store`/`key_store`/`r2_config` — provisioning it is `--init`'s job, not `--txt-ingest`'s. `content` starts `NULL`: there's nothing to encrypt until this user has at least one `txt`. `txt_metadata_key` (wrapped under the owner's `umk`, same pattern as `txt.txt_key`) protects the underlying content once it exists — a single JSON document per user rather than one row per document, `{"<txt_id>": {"name": "original filename", "metadata": {...}}, ...}` (`metadata` only present when a `<name>.opf` sidecar was found, see below) — created/updated by `--txt-ingest` (`TxtIngester._persist_metadata_update`) as each file is ingested.

  That JSON document itself lives in R2, not inline in this column: `content` holds `Blob.encrypt(txt_metadata_key, raw_path.encode("ascii"))` — `raw_path` being a fresh random R2 object key the first time this account writes it, then reused unchanged for every future update, since each user has exactly one such object (`txt/owner.py`'s `_write_txt_metadata_content` overwrites it in place — no DB write at all once the pointer's established). The R2 object body is `Blob.encrypt(txt_metadata_key, brotli(JSON), compressed=True)`, same shape the column used to hold directly before this indirection existed. Reads (`_txt_metadata_key_and_content`) distinguish the two formats purely by `content`'s byte length: a wrapped path blob is always small (~184 bytes: `BLOB_MIN_LEN` + a ~52-char base32 path), so anything at or above `constants.TXT_METADATA_LEGACY_THRESHOLD` (200 bytes) is assumed to be an account that hasn't migrated off the old inline format yet — decrypted directly (`compressed=True`) instead of treated as a path. The next write for that account (any `--txt-ingest`, or `--txt-delete-id`) migrates it: read the old way, mutate, then persist through the new (R2-backed) path.

  Once decrypted and parsed, looking up an entry by `txt_id` is an O(1) average-case dict access, but *persisting* any single change still costs O(size of the whole JSON document): there's no partial-update path, so every write decrypts (or, the first time, starts from `{}` since `content` was `NULL`), mutates, and re-encrypts the whole thing. `--txt-delete` (`txt/delete.py`'s `TxtDeleter.delete_all`) resets `content` to `NULL` once every txt is gone, deleting the R2 object too (nothing will ever point at that path again); `--txt-delete-id` (`TxtDeleter.delete_one`) instead pays the O(size of the whole document) cost to scrub just that one entry via the same overwrite-in-place path, since the rest of the account's txt survive it.
