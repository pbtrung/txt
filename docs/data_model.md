...
- **`txt`** — one row per document. Fields: `owner` (link to `$users` — always the admin account today, per the operating model above), `txtKey` (128 random bytes, wrapped under `owner`'s `umk`), and `content` — a single encrypted JSON blob (wrapped under this row's own unwrapped `txtKey`, brotli-compressed):
...
3. **The Worker must derive `scope` from the relationship between the verified identity and the requested prefix**, not just hand back whatever the client asks for — a `user` account's temporary credential may legitimately target a prefix that is _not_ their own (the admin's, for a shared document):
...
Since only the admin's `r2Prefix` is ever written to, GC is scoped to that one prefix.

1. **Document/part deletion.** Deleting a `txt` row must delete every R2 object its `txtParts` rows point at — but InstantDB's own cascade-delete (`onDelete: "cascade"` on the `txt` link) will remove those `txtParts` rows the moment the `txt` row itself is deleted, which would destroy the only record of which `raw_key`s need deleting. Order matters: first read and decrypt every `txtParts.path` for the document (recovering every `raw_key`), delete those R2 objects, and only then delete the `txt` row (letting cascade clean up `txtParts`/`txtShares` for free). A crash between the R2 deletes and the `txt` row delete leaves a real document with dangling, already-gone R2 objects — recoverable by re-running the same delete (idempotent: deleting an already-gone R2 object is a no-op).
...
- **The admin-owns-everything model concentrates blast radius under one prefix.** Every document lives under the admin's own `r2Prefix`, so the R2-credential scope-clamping logic in "Temporary, prefix-scoped R2 credentials" above is doing real security work, not just convenience scoping — a bug there exposes the whole corpus, not one account's own slice of it.
