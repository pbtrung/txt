...
CREATE TABLE IF NOT EXISTS txt_parts (
    ...
    path     BLOB    NOT NULL        -- magic||version||salt||Ascon-Keccak(path)||tag, wrapped under this txt's txt_key; path = Crockford base32 (see txt/base32.py) of HMAC-SHA3-256(txt_key, brotli(cleaned part text))
);
...
WHEN (SELECT COUNT(*) FROM bookmarks WHERE txt_id = NEW.txt_id AND user_id = NEW.user_id) >= 20
BEGIN
...
- **`txt_parts`** — a document's content, chunked into ordered parts (`part_num`, target ~200 KB per part — see `constants.PART_TARGET`) so large documents aren't loaded/decrypted as a single blob. The actual content lives in R2 object storage, not Turso: the R2 object body is `Blob.encrypt(txt_key, brotli(cleaned part text))`, and its key is `raw_path = crockford_base32(HMAC-SHA3-256(txt_key, brotli(cleaned part text)))` (`txt/base32.py` — the human-readable variant that excludes visually ambiguous I/L/O/U, no padding) — content-addressed and only computable by whoever holds `txt_key`. `path` stores that `raw_path`, itself wrapped under the owning `txt`'s `txt_key` (`Blob.encrypt(txt_key, raw_path)`) — so the object key is never visible to Turso either, only to whoever unwraps `path`. `idx_txt_parts_txt_id_part_num` supports fetching a specific part or range in order. See `txt/ingest.py` (`--add-txt`) for how raw `.txt` files are cleaned (`txt/textproc.py`), split, and uploaded.
...
- **`bookmarks`** — per-(user, txt) bookmark list, for owners and share recipients alike. The FIFO trigger caps each user at 20 bookmarks per document by evicting the oldest (lowest `id`) on overflow, so storage stays bounded without an application-side cleanup step.
- **`txt_metadata`** — one row per user (`user_id` is `UNIQUE`), created by `--init` (`AdminInitializer._ensure_txt_metadata`) alongside `umk_store`/`key_store`/`r2_config` — provisioning it is `--init`'s job, not `--add-txt`'s. `content` starts `NULL`: there's nothing to encrypt until this user has at least one `txt`. `txt_metadata_key` (wrapped under the owner's `umk`, same pattern as `txt.txt_key`) protects `content` once it exists — a single encrypted JSON blob per user rather than one row per document, `{"<txt_id>": {"name": "original filename"}, ...}` — created/updated by `--add-txt` (`TxtIngester._update_txt_metadata_entry`) as each file is ingested. Once decrypted and parsed, looking up an entry by `txt_id` is an O(1) average-case dict access, but *persisting* any single change still costs O(size of the whole blob): there's no partial-update path, so every write decrypts (or, the first time, starts from `{}` since `content` was `NULL`), mutates, and re-encrypts the entire JSON document.
