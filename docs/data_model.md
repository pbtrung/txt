...
The `creating` state is committed before upload/registration so a failed operation is resumable. A completed operation becomes `active`. Deletion changes it to `deleting`, durably tombstones the capability in D1, deletes the R2 object, then removes the row. There is no revoked state or `object_etag`: share paths are immutable, never reused, and deletion is unconditional. `ON DELETE RESTRICT` prevents deleting the source book before its shares are removed.

The Library exposes Shares below Recent only for the administrator. Shares lists the source title with Copy and Delete actions. Browse/All Books adds a Share action beside search; selecting books creates independent shares. The copied URL fragment contains the random share capability, opaque encrypted-path grant, and client-side decryption key. D1 stores only the capability hash, object-path hash, lifecycle state, and timestamps. Anonymous reading state and bookmarks remain browser-local and never mutate the owner's `db_path`.
