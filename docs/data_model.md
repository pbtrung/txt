...
- **`$users`** — one row per Firebase-authenticated identity, keyed by email. Carries two custom attributes beyond the built-in ones:
  ...
  - **`appRole`** — `'admin'` means app admin; missing/anything else is user-level. It is named `appRole` rather than `type` because `$users.type` is an InstantDB system column and cannot be changed directly by app transactions. An admin can create/update/delete any `txt`/`txtMetadata`/`txtParts`/`txtShares` row and act on any account's data, full stop; user-level accounts can only view their own `keyStore`/`credStore` rows (create/update/delete on those are admin-only — provisioning or rotating key material is a provisioning action, never a regular user self-service write), can view/create/update/delete their own `txtAccess`/`txtBookmarks` rows freely, and can read (never write) a `txt`/`txtMetadata`/`txtParts` row they have a `txtShares` grant for — see "Permission rules" below for the exact rule per entity. Only ever writable via `instant.perms.ts`'s `$users.update: "isAdmin"` — there's no `isSelf` branch, so a plain user can never touch their own `appRole` (or anything else on their own `$users` row) through the normal write path, let alone self-promote to admin.

  **Unverified — confirm before relying on this:** whether `auth.ref('$user.appRole')` (`instant.perms.ts`'s `isAdmin` check) resolves a plain, non-linked attribute on the current session's own `$users` row the same way `auth.ref`/`data.ref` resolve an attribute reached across a real link.
...
- **`credStore`** — encrypted credential rows. Fields: `owner` (link to `$users` — the account whose `umk` wraps this row's `credStoreKey`), `forUser` (link to `$users` — the account these credentials belong to), `credStoreKey` (128 random bytes, freshly generated per row and wrapped under `owner`'s `umk`), and `content`, a single encrypted JSON blob protected by that row's own `credStoreKey`. `forUser` is deliberately plaintext and queryable: the admin user-management flow can fetch the admin-owned credential row for one target user directly, instead of decrypting every admin-owned `credStore` row and matching by email or embedded id. It is not a permission grant; read access is still based on `owner` (or `isAdmin`) below.
  ...
  A `user`-role row's `r2_config` carries no access keys at all, since a `user` session never holds a static R2 credential of any kind, only a short-lived, prefix-scoped, read-only temporary one minted on demand (see [r2_credentials.md](r2_credentials.md)) — a `user` account never writes to R2, and the Worker never grants it a credential that could. The admin's row carries the real, permanent R2 keys too, but only CLI tooling (`--migrate`, `--clean-bucket`, `--collect-garbage`, document ingest) ever reads them directly — `ui/`, even in an admin-logged-in session, never builds an R2 client from this row's own keys, only ever from a Worker-minted temporary credential like any other session (see [r2_credentials.md](r2_credentials.md)).
...
- `isAdmin` — `'admin' in auth.ref('$user.appRole')`.
