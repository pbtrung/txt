...
5. `await db.storage.uploadFile(pageKey, <the encrypted raw_path from step 4>)`, where `pageKey = "${auth.id}:${pageNo}:${version}"` (the same value `pages.pageKey` will use). This _creates_ the `$files` row (`path = pageKey`). This is the only way to create a `$files` row and can't be folded into the `transact()` below; ownership at this point is enforced purely by `instant.perms.ts` checking `data.path.startsWith(auth.id + ':')`, which `pageKey` already satisfies, since no link to any other entity exists yet.
