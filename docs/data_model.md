...
- **The commit pattern** (not a table — how `pages` and `db_meta` are written together). A client-side SQLite commit buffers all dirty pages in memory, then flushes them as one atomic multi-statement rqlite transaction:
  ...
  -- column1/column2/... are SQLite's own default names for an anonymous
  -- VALUES-derived table -- NOT an explicit "AS dirty(db_id, page_no, ...)"
  -- column-naming alias, which is a newer SQLite grammar addition than the
  -- version this deployment's rqlite bundles and fails with a silent (see
  -- below) "near '(': syntax error".
  ...
  The `WHERE current_version=old_N` clause on the `UPDATE` is the concurrency-control mechanism: the client read `old_N` before building this transaction, and if the `UPDATE` reports 0 rows affected, some other writer committed first, so this client must rebuild its dirty pages against the new base version and retry. Both statements are sent as one `/db/execute?transaction` request so rqlite applies them as a single atomic unit with no other transaction able to interleave (**the `?transaction` query parameter is required for this** — without it rqlite doesn't guarantee the two statements execute together, which is exactly how the `INSERT`'s now-fixed syntax error above went undetected for so long: the separately-executed `UPDATE` kept succeeding and advancing `current_version` on its own even while every `INSERT` failed) — which is why the `INSERT` needs its own copy of the same guard rather than an unconditional `VALUES (...)`: an `UPDATE` matching zero rows is a normal, successful no-op in SQLite, not an error, so without the matching guard on the `INSERT` a writer that loses the CAS race would still have its page rows land, stamped with a version number it never actually won — silently corrupting that version for any reader who later reads a page only the loser touched. Guarding both statements identically means a lost race makes the whole transaction a no-op, together, at the cost of one indexed subquery per statement instead of a flat `VALUES` list. The client checks **both** statements' own results, not just the `UPDATE`'s — a lesson from the syntax error above: a caller that only checks the `UPDATE` can't tell a genuinely-applied commit apart from one where the `INSERT` silently failed underneath it.
