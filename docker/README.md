...
`ui:build` (see the root `package.json`) needs `ui/build-creds.json` -- a small, gitignored, deployment-owned config (`asset_base_url`, `rqlite_host`, and a `slhdsa_256f_priv_key` it generates and writes back on first use if absent), distinct from any end user's own vault creds file (`ui/src/data/creds.ts`'s `{rqlite_url, api_key, user_root_key, r2_config}`). Defaults to that path; point it at a different file with `npm run ui:build -- path/to/creds.json` (or `--build-creds path/to/creds.json`) -- either way, a missing `slhdsa_256f_priv_key` in that file gets generated and written back, an existing one is always reused. See `ui/scripts/build-integrity.mjs`'s own header comment for exactly what each field does.
