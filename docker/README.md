...
`ui:build` (see the root `package.json`) needs `ui/build-creds.json` -- a small, gitignored, deployment-owned config (`asset_base_url`, `rqlite_url`, and a `slhdsa_256f_priv_key` it generates and writes back on first use if absent), distinct from any end user's own vault creds file (`ui/src/data/creds.ts`'s `{rqlite_url, api_key, user_root_key}` -- `rqlite_url` reuses that same field name for the same concept, even though this is a separate, deployment-level fact; that account's R2 credentials live in its own SQLCipher db's `r2_config` table instead, not in this creds file). `rqlite_url`'s origin backs `dist/_headers`' CSP `connect-src`. Defaults to that path; point it at a different file with `npm run ui:build -- path/to/creds.json` (or `--build-creds path/to/creds.json`) -- either way, a missing `slhdsa_256f_priv_key` in that file gets generated and written back, an existing one is always reused. See `ui/scripts/build-integrity.mjs`'s own header comment for exactly what each field does.

Deploy `dist/` as a Cloudflare Pages project (`wrangler pages deploy dist/`, or connect the repo and set the build output directory to `dist/` -- `npm run ui:build` as the build command). Cloudflare Pages reads `dist/_headers` natively, written by `build-integrity.mjs`:

- **`_headers`** sets, for every path: the direct-CDN-visit CSP (narrowed `connect-src`, see above), `Access-Control-Allow-Origin: null` -- the literal string `null`, not a wildcard `*` -- and `Cross-Origin-Opener-Policy: same-origin` / `Cross-Origin-Embedder-Policy: credentialless`. `local_index.html`, opened via `file://`, sends `Origin: null` on its cross-origin fetches to `manifest.json`/`manifest.sig`/every other `dist/` asset; without a response header that matches that exact literal value, the browser blocks reading the response body even though the request itself succeeds. A wildcard `*` does **not** reliably substitute for this in practice for a `null`-origin request the way it does for a real origin -- confirmed the hard way (see git history: this project briefly regressed to `*` during a rewrite, which silently broke `local_index.html` verification from `file://` until traced back to this exact header). Safe to allow broadly: these are public, non-secret build outputs whose integrity `local_index.html` itself checks via SLH-DSA/SHA-512, not via keeping them cross-origin-unreadable. The COOP/COEP pair is what makes `SharedArrayBuffer` available at all -- `ui/src/data/dbWorker.ts`'s Worker + `SharedArrayBuffer` + `Atomics.wait` bridge needs it, and browsers only expose `SharedArrayBuffer` to a page that's cross-origin isolated. `credentialless`, not the stricter `require-corp`: `require-corp` would also block the app's own cross-origin `fetch`es to the rqlite API and to R2, since neither service necessarily sends back a `Cross-Origin-Resource-Policy` header of its own. There's no separate server config to set these on Cloudflare Pages -- this file is the only place they're set.
