...
Verification's own `fetch()` calls can also fail outright from a bare `file://` origin on desktop (Chrome reports this as a DevTools "Issue": `'file:' URLs are treated as unique security origins`) -- `_headers`' wildcard `Access-Control-Allow-Origin: *` above doesn't reliably fix this, since a `file://`-origin request doesn't send a normal `Origin` header for that wildcard to match against. `ui/src/localIndex/verify.ts`'s `fetchFailureError` detects this specific case and points the user at the actual fix: serve `local_index.html` over a trivial local HTTP server instead (e.g. `python3 -m http.server` from the folder it's in, then open `http://localhost:8000/local_index.html`) -- a normal `http://localhost` origin, where the same wildcard header works exactly as intended. This doesn't apply to `content://` on Android/iOS, which isn't a browser-enforced fetch restriction the same way.
